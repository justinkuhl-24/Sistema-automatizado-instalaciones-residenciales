...
- Empalme máximo 40 [A] (tipo A-9 o S-9), en caso de que supere esta demanda se emite aviso y se recomienda revisar de forma manual los cálculos.
- Solo monofásico 220 [V], no contempla instalaciones trifásicas.
- Hasta 10 circuitos.
- Los datos se ingresan por consola en cada ejecución, así que un error advertido al final obliga a reiniciar.
- La cubicación entrega cantidades, no valorización. No incluye precio.
- El diámetro Ø conduit PVC para el alimentador es implementada en conductores de hasta 16 [mm²], es decir, que para secciones mayores retornan el máximo de la tabla.
- El diámetro Ø conduit PVC para la canalización subterránea es implementada hasta conductores hasta 25 [mm²] y máximo 3 conductores.
- La selectividad está considerada en base a lo que dice la normativa (curva B en iluminación, curva C en enchufes, curva D en el empalme), no está diseñada mediante curvas gráficas.
- Todos los cálculos utilizan rho igual a 0,0179 [Ω × mm²/m]. No contempla aluminio.
- Sin interfaz gráfica, toda la interacción es por consola.
- El LRA (Corriente de rotor bloqueado "Locked Rotor Amps") del compresor del aire acondicionado decide si hay que subir la sección del conductor. Los equipos inverter usan curva C (arranque suave, dispara entre 5 - 10 la corriente nominal). Los equipos on/off primero prueban con una curva C, y solo si el LRA no alcanza a cubrirse con ningún calibre, pasan a curva D, ya que, dispara entre 10 - 20 veces la corriente nominal, posee un arranque brusco y debe tolerar más corriente sin que el interruptor termomagnético se dispare antes de tiempo), pero esto no reemplaza un estudio de arranque completo para motores grandes.
- Marco normativo utilizado (RIC N°1, 2, 3, 4, 5, 6, 7, 10, 11).
