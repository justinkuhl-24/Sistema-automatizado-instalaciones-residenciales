...
| Tablero | KW | In [A] | f/d | KW | In [A] |
|---|---|---|---|---|---|
| T.D.A PRIMEROS 3KW | sin fd | sin fd | 1,00 | con fd | con fd |
| T.D.A RESTO x KW | sin fd | sin fd | 0,35 | con fd | con fd |
| Una fila por equipo de climatización | | | 1,00 | | |
| Una fila por equipo de agua caliente | | | 1,00 | | |
| TOTAL | suma | suma | | suma | suma |

Las filas de climatización y agua caliente son dinámicas: aparece una por cada equipo declarado, con su nombre, y siempre con factor de demanda 1,00, porque el RIC 7 no permite reducir esas cargas. La fila TOTAL de la última columna es la corriente que después se multiplica por 1,10 para elegir el calibre del empalme.
...
Para elegir el termomagnético, el programa sigue esta lógica. Si el fabricante declara un MOCP, ese valor manda y no se supera, según RIC 5.6.2.3. Si no lo declara, se toma el calibre comercial inmediatamente superior a la corriente de diseño. La curva es C para equipos inverter, que tienen arranque suave de 5 a 10 veces la corriente nominal, y D para equipos on-off, con arranque de 10 a 20 veces. Si el LRA declarado supera el umbral magnético de la curva elegida, se prueba primero con curva D antes de subir de calibre.
...
| Termomagnético especial | In<sub>TM</sub> ≥ 1,10 × I<sub>circ</sub>, mín. 16 A | 10% de reserva de capacidad. Mínimo 16 A en agua caliente y climatización |
...
| Sección mínima normativa | S ≥ 1,5 mm² ilum. / S ≥ 2,5 mm² ench. | Piso que se aplica aunque el cálculo dé menos. RIC 4 |
...
| Fórmula | Expresión | Variables |
|---|---|---|
| Factor de demanda | P<sub>d</sub> = P<sub>1</sub> × 1,0 + P<sub>2</sub> × 0,35 | P<sub>1</sub>=mín(P<sub>total</sub>, 3000) [W] primeros 3 kW al 100%, P<sub>2</sub>=máx(P<sub>total</sub> − 3000, 0) [W] resto al 35%. RIC 3, art. 6.1, 6.2 y 6.3 |
...
**Solo monofásico.** El programa está construido para instalaciones residenciales de 1F+N+PE a 220 V. No calcula sistemas trifásicos.

**Empalme hasta 40 A.** Está calibrado para empalmes tipo A-9 y S-9. Sobre ese valor sigue calculando, pero emite un aviso de revisión manual.

**Máximo 10 circuitos.** Es el límite de diseño del tablero considerado.

**Sin persistencia de datos.** Los datos se ingresan por consola en cada ejecución. No hay guardado ni recarga de proyectos, así que un error advertido al final obliga a reiniciar.

**Sin precios.** La cubicación entrega cantidades, no valorización.

**Tablas parciales.** La Tabla N°4.17 está implementada hasta 16 mm² y 5 conductores. La Tabla N°4.29 llega hasta 25 mm² y 3 conductores.

**Conductor de cobre.** Todos los cálculos usan rho igual a 0,0179 ohm por mm² por metro. No contempla aluminio.

**Potencias estimadas.** Cuando el usuario no conoce la potencia de una luminaria, se usa un valor típico según tipo y montaje.
